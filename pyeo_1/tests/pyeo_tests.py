...
import pyeo_1.coordinate_manipulation
...
def test_stack_images(managed_multiple_geotiff_dir):
    ...
    pyeo_1.raster_manipulation.stack_images(images, result_path)
...
def test_stack_and_trim_images(managed_noncontiguous_geotiff_dir):
    ...
    pyeo_1.raster_manipulation.stack_and_trim_images(old_image_path, new_image_path, aoi_path, result_path)
...
def test_multiple_union():
    ...
    out = pyeo_1.coordinate_manipulation.multiple_union(test_polys)
...
def test_pixel_bounds_from_polygon(managed_noncontiguous_geotiff_dir):
    ...
    result = pyeo_1.coordinate_manipulation.pixel_bounds_from_polygon(raster, polygon)
...
def test_point_to_pixel_coordinates(managed_noncontiguous_geotiff_dir):
    ...
    raster = gdal.Open(os.path.join(test_dir.path, "ne_test"))
    out = pyeo_1.coordinate_manipulation.point_to_pixel_coordinates(raster, point)
...
def test_multiple_intersection():
    ...
    out = pyeo_1.coordinate_manipulation.multiple_intersection(test_polys)
    assert out.__str__() == target.__str__()
...
def test_clip_raster(managed_noncontiguous_geotiff_dir):
    ...
    result_path = os.path.join(test_dir.path, "test_out.tif")
    pyeo_1.raster_manipulation.clip_raster(test_raster, test_aoi, result_path)
    result = gdal.Open(result_path)
...
def test_write_polygon(managed_geotiff_dir):
    ...
    pyeo_1.write_polygon(poly, result_path)
...
def test_check_overlap(managed_noncontiguous_geotiff_dir):
    ...
    assert pyeo_1.coordinate_manipulation.check_overlap(test_raster, test_aoi) is True
...
def test_get_raster_bounds(managed_noncontiguous_geotiff_dir):
    ...
    test_raster = gdal.Open(os.path.join(test_dir.path, "ne_test"))
    target = (0, 110, 0, 120)
    result = pyeo_1.coordinate_manipulation.get_raster_bounds(test_raster)
...
def test_get_aoi_bounds(managed_noncontiguous_geotiff_dir):
    ...
    test_aoi = ogr.Open(os.path.join(test_dir.path, "aoi"))
    target = (10, 20, 10, 20)
    result = pyeo_1.coordinate_manipulation.get_aoi_bounds(test_aoi)
    assert result.GetEnvelope() == target
...
def test_get_aoi_intersection(managed_noncontiguous_geotiff_dir):
    ...
    test_aoi = ogr.Open(os.path.join(test_dir.path, "aoi"))
    result = pyeo_1.coordinate_manipulation.get_aoi_intersection(test_raster, test_aoi)
...
def test_aoi_to_mask(managed_noncontiguous_geotiff_dir):
    ...
    test_aoi = ogr.Open(os.path.join(test_dir.path, "aoi"))
    result_path = os.path.join(test_dir.path, "test_out.tif")
    pyeo_1.aoi_to_mask(test_aoi, result_path)
    result = gdal.Open(result_path)
...
def test_classify_change(managed_ml_geotiff_dir):
    ...
    model = None
    pyeo_1.classification.classify_image(test_path, model, test_dir.path, test_dir.path)
...
def test_reshape_raster_for_ml(managed_ml_geotiff_dir):
    ...
    array = pyeo_1.classification.reshape_raster_for_ml(array)
...
def test_reshape_ml_out_to_raster():
    ...
    out = pyeo_1.classification.reshape_ml_out_to_raster(ml_array, 10, 6)
...
def test_get_training_data(managed_ml_geotiff_dir):
    ...
    out = pyeo_1.classification.get_training_data(training_image, training_shapes)
...
def test_sort_by_timestamp():
    with TemporaryDirectory() as td:
        ...
        out_paths = pyeo_1.sort_by_s2_timestamp(input)
