...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "l1_dir", action="store", help="Path to the directory containing L1 imagery"
    )
    parser.add_argument(
        "l2_dir", action="store", help="Path to directory to contain L2 imagery"
    )
    array_id = int(os.getenv("PBS_ARRAYID"))
    sen_2_cor_home = os.getenv("SEN2COR_HOME")
    args = parser.parse_args()

    log = pyeo_1.filesystem_utilities.init_log("sen2cor_{}.log".format(array_id))
    ...
    file_list = [
        os.path.join(args.l1_dir, l1_filename)
        for l1_filename in sorted(os.listdir(args.l1_dir))
    ]

    l2_name = pyeo_1.raster_manipulation.apply_sen2cor(
        file_list[array_id],
