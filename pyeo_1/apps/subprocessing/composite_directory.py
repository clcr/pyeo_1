...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Composites a directory of .tifs and .msks, creating an image of the "
        "latest cloud-free pixels."
    )
    parser.add_argument(
        "in_dir", action="store", help="Path to the directory to be composited"
    )
    ...
    pyeo_1.filesystem_utilities.init_log(args.logpath)

    if args.merge_path:
        comp_dir = args.merge_path
        for safe_file in [
            os.path.join(os.path.dirname(args.in_dir), file)
            for file in os.listdir(args.in_dir)
        ]:
            pyeo_1.raster_manipulation.stack_sentinel_2_bands(safe_file, comp_dir)

    if args.mask_path:
        for image in [
            os.path.join(os.path.dirname(comp_dir), file)
            for file in os.listdir(comp_dir)
        ]:
            pyeo_1.raster_manipulation.create_mask_from_model(image, args.mask_path)

    pyeo_1.raster_manipulation.composite_directory(
        comp_dir, args.out_path, generate_date_images=args.generate_dates_image
    )
