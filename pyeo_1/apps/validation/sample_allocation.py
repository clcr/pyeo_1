...
def main():
    parser = argparse.ArgumentParser(
        "Sample allocation: given a specification in a .ini file,"
    ...
    if not os.path.exists(args.conf_path):
        raise FileNotFoundError(
            "No configuration found at {}, please check path to .ini file".format(
                args.conf_path
            )
        )
    ...
    user_accuracies = {
        int(map_class): float(number) for map_class, number in user_accuracies.items()
    }
    if "pinned_samples" in conf:
        pinned_samples = conf["pinned_samples"]
        pinned_samples = {
            int(map_class): int(number) for map_class, number in pinned_samples.items()
        }
    else:
        pinned_samples = None
