...
def rolling_detection(config_path,
    ...
    dataspace_username = str(conf['pyeo2_flow']['dataspace_username'])
    dataspace_password = str(conf['pyeo2_flow']['dataspace_password'])
    tiles_to_process = str(conf['pyeo2_flow']['tiles_to_process'])
    ...
    try:
        ...
        if do_all or build_composite:
            if use_new_api:
                image_acquisition_pipeline.download(
                    ...
                    safe_download_path=composite_l2_image_dir,
                ...
                if len(l2a_zip_file_paths) > 0:
                    for f in l2a_zip_file_paths:
                        ...
                        zip_timestamp = pyeo_1.filesystem_utilities.get_image_acquisition_time(os.path.basename(f)).strftime("%Y%m%dT%H%M%S")
                ...
                if len(l2a_safe_file_paths) > 0:
                    for f in l2a_safe_file_paths:
                        ...
                        safe_timestamp  = pyeo_1.filesystem_utilities.get_image_acquisition_time(os.path.basename(f)).strftime("%Y%m%dT%H%M%S")
                ...
                if len(files_for_cloud_masking) == 0:
                    ...
                    pyeo_1.raster_manipulation.apply_scl_cloud_mask(composite_l2_image_dir,
                    ...
                    pyeo_1.raster_manipulation.apply_processing_baseline_offset_correction_to_tiff_file_directory(composite_l2_masked_image_dir, composite_l2_masked_image_dir)
                    ...
                    for main_dir in dirs_for_quicklooks:
                        ...
                        if len(files) == 0:
                            ...
                            for f in files:
                                ...
                                pyeo_1.raster_manipulation.create_quicklook(f,
                                                                          quicklook_path,
                                                                          width=512,
                                                                          height=512,
                                                                          format="PNG",
            ...
            if len(masked_file_paths) > 0:
                pyeo_1.raster_manipulation.clever_composite_directory(composite_l2_masked_image_dir,
                                                                    composite_dir,
                                                                    chunks=chunks,
                ...
                for root, dirs, files in os.walk(composite_dir):
                    ...
                    for this_tiff in all_tiffs:
                        pyeo_1.raster_manipulation.compress_tiff(os.path.join(root, this_tiff), os.path.join(root, this_tiff))
        ...
        if do_all or do_download:
            if use_new_api:
                image_acquisition_pipeline.download(geom_path=tiles_to_process,
                                                    max_cloud_cover=cloud_cover,
                                                    start_date=start_date,
                                                    end_date=end_date,
                                                    safe_download_path=l2_image_dir,
                                                    username=dataspace_username,
                                                    password=dataspace_password)
            else:
                log.info("---------------------------------------------------------------")
                ...
                products_all = pyeo_1.queries_and_downloads.check_for_s2_data_by_date(root_dir,
                                                                                   start_date,
                                                                                   end_date,
                                                                                   conf,
                                                                                   cloud_cover=cloud_cover,
                ...
                if l1c_products.shape[0] > 0:
                    ...
                    if (l1c_products.shape[0] >0):
                        ...
                        pyeo_1.queries_and_downloads.download_s2_data_from_df(l1c_products,
                                                                    ...
                                                                    try_scihub_on_fail=True)
                        log.info("Atmospheric correction with sen2cor.")
                        pyeo_1.raster_manipulation.atmospheric_correction(l1_image_dir,
                                                                        l2_image_dir,
                                                                        sen2cor_path,
                                                                        delete_unprocessed_image=False)
                if l2a_products.shape[0] > 0:
                ...
                incomplete_downloads, sizes = pyeo_1.raster_manipulation.find_small_safe_dirs(l2_image_dir, threshold=faulty_granule_threshold*1024*1024)
                if len(incomplete_downloads) > 0:
                    for index, safe_dir in enumerate(incomplete_downloads):
                        if sizes[index]/1024/1024 < faulty_granule_threshold and os.path.exists(safe_dir):
                            log.warning("Found likely incomplete download of size {} MB: {}".format(str(round(sizes[index]/1024/1024)), safe_dir))
            ...
            pyeo_1.raster_manipulation.apply_scl_cloud_mask(l2_image_dir,
                                                          l2_masked_image_dir,
                                                          scl_classes=[0,1,2,3,8,9,10,11],
            ...
            pyeo_1.raster_manipulation.apply_processing_baseline_offset_correction_to_tiff_file_directory(l2_masked_image_dir, l2_masked_image_dir)
        ...
        if do_all or do_classify:
            ...
            pyeo_1.classification.classify_directory(composite_dir,
                                                   model_path,
                                                   ...
                                                   chunks=chunks,
                                                   skip_existing=skip_existing)
            pyeo_1.classification.classify_directory(l2_masked_image_dir,
                                                   model_path,
                                                   categorised_image_dir,
                                                   prob_out_dir = None,
                                                   apply_mask=False,
        ...
        if do_all or do_change:
            ...
            if sieve > 0:
                ...
                sieved_paths = pyeo_1.raster_manipulation.sieve_directory(in_dir = categorised_image_dir,
            ...
            if not do_dev:
                ...
                pyeo_1.raster_manipulation.combine_date_maps(date_image_paths, output_product)

            log.info("---------------------------------------------------------------")
            log.info("Report image product completed / updated: {}".format(output_product))
            log.info("Compressing the report image.")
            log.info("---------------------------------------------------------------")
            pyeo_1.raster_manipulation.compress_tiff(output_product, output_product)
...
if __name__ == "__main__":
    ...
    parser.add_argument(dest='config_path', action='store', default=r'change_detection.ini',
                        help="A path to a .ini file containing the specification for the job. See "
                             "pyeo_1/apps/change_detection/change_detection.ini for an example.")
