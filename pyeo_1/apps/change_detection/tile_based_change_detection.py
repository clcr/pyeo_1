...
def rolling_detection(
    ...
    log = pyeo_1.filesystem_utilities.init_log(
    ...
    try:
        ...
        if arg_start_date == "LATEST":
            ...
            start_date = pyeo_1.filesystem_utilities.get_image_acquisition_time(
        ...
        if build_composite or do_all:
            ...
            composite_products_all = (
                pyeo_1.queries_and_downloads.check_for_s2_data_by_date(
                    root_dir,
            ...
            if l1c_products.shape[0] > 0 and l2a_products.shape[0] > 0:
                ...
                (
                    l1c_products,
                    l2a_products,
                ) = pyeo_1.queries_and_downloads.filter_unique_l1c_and_l2a_data(df)
                log.info(
            ...
            if l1c_products.shape[0] > 0:
                ...
                for r in range(n):
                    ...
                    matching_l2a_products = (
                        pyeo_1.queries_and_downloads._file_api_query(
                            user=sen_user,
                            passwd=sen_pass,
                ...
                pyeo_1.queries_and_downloads.download_s2_data_from_df(
                    l1c_products,
                ...
                pyeo_1.raster_manipulation.atmospheric_correction(
                    composite_l1_image_dir,
                    composite_l2_image_dir,
                    sen2cor_path,
            ...
            if l2a_products.shape[0] > 0:
                ...
                pyeo_1.queries_and_downloads.download_s2_data(
            ...
            # raster_paths = pyeo_1.filesystem_utilities.get_raster_paths(l2a_paths, filepatterns=bands, dirpattern=resolution) # don't really need to know these
            # scl_raster_paths = pyeo_1.filesystem_utilities.get_raster_paths(l2a_paths, filepatterns=["SCL"], dirpattern="20m") # don't really need to know these

            log.info(
                "Applying simple cloud and cloud shadow mask based on SCL files and stacking the masked band raster files."
            )
            pyeo_1.raster_manipulation.apply_scl_cloud_mask(
                composite_l2_image_dir,
            ...
            pyeo_1.raster_manipulation.clever_composite_directory(
                composite_l2_masked_image_dir,
                composite_dir,
                chunks=5,
        ...
        if do_all or do_download:
            ...
            products_all = pyeo_1.queries_and_downloads.check_for_s2_data_by_date(
                root_dir,
                start_date,
                end_date,
                conf,
        ...
        if do_all or do_classify:
            ...
            images = pyeo_1.filesystem_utilities.sort_by_timestamp(
                [
                    image_name
            ...
            try:
                latest_composite_name = pyeo_1.filesystem_utilities.sort_by_timestamp(
                    [
                        image_name
            ...
            for image in images:
                ...
                before_timestamp = (
                    pyeo_1.filesystem_utilities.get_image_acquisition_time(
                ...
                if before_timestamp < after_timestamp:
                    before_timestamp_str = (
                        pyeo_1.filesystem_utilities.get_sen_2_image_timestamp(
                            latest_composite_name
                        )
                    )
                    after_timestamp_str = (
                    ...
                    if build_prob_image:
                        ...
                        new_prob_image = None
                    pyeo_1.classification.change_from_composite(
            ...
            class_image_paths = list(
                filter(
                    pyeo_1.filesystem_utilities.get_change_detection_dates,
                    class_image_paths,
                )
            )
            class_image_paths.sort(
                key=lambda x: pyeo_1.filesystem_utilities.get_change_detection_dates(x)[
                    1
                ]
            )
            ...
            for index, image in enumerate(class_image_paths, start=n_confirmations):
                ...
                pyeo_1.raster_manipulation.verify_change_detections(
                    subsequent_images,
                    verification_raster,
                    [5],
                    buffer_size=0,
        ...
        if do_update or do_all:
            ...
            composite_images = pyeo_1.filesystem_utilities.sort_by_timestamp(
                [
                    image_name
                    for image_name in os.listdir(composite_dir)
                    if image_name.endswith(".tif")
            ...
            categorised_images = pyeo_1.filesystem_utilities.sort_by_timestamp(
                [
                    image_name
                    for image_name in os.listdir(categorised_image_dir)
                    if image_name.endswith(".tif")
            ...
            categorised_images = [
                ...
                if pyeo_1.filesystem_utilities.get_change_detection_dates(
                    os.path.basename(image)
                )[1]
                > latest_composite_timestamp_datetime
            ]
            ...
            log.info("Sorting masked L2A image list by time stamp.")
            l2a_images = pyeo_1.filesystem_utilities.sort_by_timestamp(
                [
                    image_name
                    for image_name in os.listdir(l2_masked_image_dir)
                    if image_name.endswith(".tif")
            ...
            for categorised_image in categorised_images:
                ...
                timestamp = (
                    pyeo_1.filesystem_utilities.get_change_detection_date_strings(
                        os.path.basename(categorised_image)
                    )
                )
                before_time = timestamp[0]
                after_time = timestamp[1]
                granule = pyeo_1.filesystem_utilities.get_sen_2_image_tile(
                    os.path.basename(categorised_image)
                )
                ...
                with TemporaryDirectory(dir=os.getcwd()) as td:
                    ...
                    pyeo_1.raster_manipulation.create_mask_from_class_map(
                        os.path.join(categorised_image_dir, categorised_image),
                        mask_path,
                        [class_of_interest],
                        buffer_size=0,
                    ...
                    pyeo_1.raster_manipulation.apply_mask_to_image(
            ...
            for main_dir in dirs_for_quicklooks:
                ...
                if len(files) == 0:
                    ...
                    for f in files:
                        ...
                        pyeo_1.raster_manipulation.create_quicklook(
                            f,
                            quicklook_path,
                            width=512,
                            height=512,
