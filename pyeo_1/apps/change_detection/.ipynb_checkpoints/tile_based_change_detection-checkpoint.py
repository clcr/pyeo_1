    ...
    try:
        ...
        if do_all or do_classify:
            ...
            for image in images:
                ...
                before_timestamp = pyeo_1.filesystem_utilities.get_image_acquisition_time(latest_composite_name)
                ...
                if before_timestamp < after_timestamp:
                    before_timestamp_str = pyeo_1.filesystem_utilities.get_sen_2_image_timestamp(latest_composite_name)
                    after_timestamp_str = pyeo_1.filesystem_utilities.get_sen_2_image_timestamp(os.path.basename(image))
                    ...
                    if build_prob_image:
                        ...
                        new_prob_image = None
                    pyeo_1.classification.change_from_composite(os.path.join(l2_masked_image_dir, image), 
            ...
            class_image_paths = list(filter(pyeo_1.filesystem_utilities.get_change_detection_dates, class_image_paths))
            class_image_paths.sort(key=lambda x: pyeo_1.filesystem_utilities.get_change_detection_dates(x)[1])

            for index, image in enumerate(class_image_paths):
                log.info("{}: {}".format(index, image))
            for index, image in enumerate(class_image_paths):
                log.info(pyeo_1.filesystem_utilities.get_change_detection_dates(os.path.basename(image))[1])
            ...
            for index, image in enumerate(class_image_paths, start=n_confirmations):
                ...
                #   (new_class_image should have the before and after dates/timestamps in the filename)
                before_timestamp = pyeo_1.filesystem_utilities.get_change_detection_dates(os.path.basename(subsequent_images[0]))[0]
                after_timestamp  = pyeo_1.filesystem_utilities.get_change_detection_dates(os.path.basename(subsequent_images[-1]))[1]
                ...
                pyeo_1.raster_manipulation.verify_change_detections(subsequent_images, verification_raster, [5], buffer_size=0, out_resolution=None)
