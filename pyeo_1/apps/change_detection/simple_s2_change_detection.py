...
::
   ...
   $ python pyeo_1/apps/change_detection/simple_s2_change_detection.py --conf /path/to/change_detection.ini
...
if __name__ == "__main__":
    ...
    if args.do_download or do_all:
        ...
        products = pyeo_1.queries_and_downloads.check_for_s2_data_by_date(aoi_path, start_date, end_date, conf)
        log.info("Downloading Sentinel-2 L1 and L2 images")
        pyeo_1.queries_and_downloads.download_s2_data(products, l1_image_path, l2_image_path, source='aws')
    ...
    if args.do_preprocess or do_all:
        ...
        pyeo_1.raster_manipulation.atmospheric_correction(l1_image_path, l2_image_path, sen2cor_path, delete_unprocessed_image=False)
    ...
    if args.do_merge or do_all:
        ...
        pyeo_1.filesystem_utilities.clean_l2_dir(l2_image_path, resolution="10m", warning=False)
        log.info("Merging all 10m band files into single Geotiffs, creating cloud masks and reprojecting output images")
        pyeo_1.raster_manipulation.preprocess_sen2_images(l2_image_path, merged_image_path, cloud_certainty_threshold)

    # Stack layers
    if args.do_stack or do_all:
        log.info("-----------------------------------------------------")
        log.info("Stacking before and after images")
        pyeo_1.raster_manipulation.create_new_stacks(merged_image_path, stacked_image_path)

    # Mosaic stacked layers
    if args.do_mosaic or do_all:
        log.info("-----------------------------------------------------")
        log.info("Mosaicking stacked multitemporal images across tiles")
        pyeo_1.raster_manipulation.mosaic_images(stacked_image_path, mosaic_image_path, format="GTiff", datatype=gdal.GDT_Int32, nodata=0)

    # Classify stacks
    if args.do_classify or do_all:
        log.info("-----------------------------------------------------")
        log.info("Classifying images")
        pyeo_1.classification.classify_directory(stacked_image_path, model_path, catagorised_image_path, probability_image_path,
