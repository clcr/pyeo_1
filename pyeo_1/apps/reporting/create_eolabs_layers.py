...
def create_report(class_path, out_path, class_color_key=DEFAULT_KEY, certainty_path=None):
    ...
    if certainty_path:
        pyeo_1.raster_manipulation.flatten_probability_image(certainty_path, os.path.join(out_dir, "prob.tif"))
...
def create_display_layer(class_path, out_path, class_color_key):
    ...
    display_raster = pyeo_1.raster_manipulation.create_matching_dataset(class_raster, out_path, bands=3, datatype=gdal.GDT_Byte)
