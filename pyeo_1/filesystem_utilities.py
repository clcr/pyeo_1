...
def gdal_switch(installation: str,
    ...
    If the environment_manager is not set to `conda` in the `.ini`, then this function does not run, this is by design as the venv installation of pyeo does not require multiple GDAL installations.
...
def get_change_detection_date_strings(image_name):
    ...
    ----------
    image_name : str
        A pyeo_1-produced image name (example: `class_composite_T36MZE_20190509T073621_20190519T073621.tif`)
...
def get_preceding_image_path(target_image_name, search_dir):
    ...
    target_image_name : str
        A pyeo_1 or Sentinel generated image name
    search_dir : str
...
def get_pyeo_1_timestamp(image_name):
    """
    Returns a list of all timestamps in a pyeo_1 image (yyyymmddhhmmss)

    Parameters
    ----------
    image_name : str
        The pyeo_1 image name
