...
def vectorise_from_band(change_report_path: str, band: int):
    ...
    with TemporaryDirectory(dir=os.getcwd()) as td:
        ...
        try:
            ...
            log.error(f"Could not open band {band}. encountered the following error \n {error}")
...
def filter_vectorised_band(vectorised_band_path: str):
    ...
    os.environ["PROJ_LIB"] = f"{home}/miniconda3/envs/pyeo_prod_0_8_0_env/lib/python3.10/site-packages/pyproj/proj_dir/share/proj"
...
def zonal_statistics(raster_path: str, shapefile_path: str, report_band: int):
    ...
    while p_feat:
        ...
        try:
            ...
            if p_feat.GetGeometryRef() is not None:
                ...
                tp_lyr = tp_ds.CreateLayer('polygons', srs=proj, geom_type=ogr.wkbPolygon)
    ...
    fn_csv = f"{os.path.splitext(raster_path)[0]}_zstats_over_{band_naming(report_band)}.csv"
...
def geopandas_function(
    ...
    home = str(Path.home())
    os.environ["GDAL_DATA"] = f"{home}/miniconda3/envs/pyeo_prod_0_8_0_env/lib/python3.10/site-packages/pyproj/proj_dir/share/gdal"
    os.environ["PROJ_LIB"] = f"{home}/miniconda3/envs/pyeo_prod_0_8_0_env/lib/python3.10/site-packages/pyproj/proj_dir/share/proj"
    ...
    log.info(f"reading in administrative boundary information from {level_1_boundaries_path}")
    boundaries = gpd.read_file(level_1_boundaries_path)
    boundaries = boundaries.filter(["NAME_1", "geometry"]).rename(columns = {"NAME_1": "County"})

    # check crs logic
    if boundaries.crs is not epsg:
        log.info(f"boundary epsg is {boundaries.crs}, but merged dataframe has {merged.crs}")
    ...
    # county spatial join
    merged = merged.sjoin(boundaries, predicate = "within", how = "left").drop(["index_right"], axis = 1)
...
def vector_report_generation(raster_change_report_path: str, write_csv: bool, write_shapefile: bool):
    """
    ...
    # 22 minutes
    path_vectorised_binary = vectorise_from_band(change_report_path=change_report_path, band=6)
    
    # 1.5 minutes
    path_vectorised_binary_filtered = filter_vectorised_band(vectorised_band_path = path_vectorised_binary)

    # 6 minutes
    rb2_zstats_df = zonal_statistics(raster_path=change_report_path, shapefile_path = path_vectorised_binary_filtered, report_band=2)
    
    # 6 minutes
    rb5_zstats_df = zonal_statistics(raster_path=change_report_path, shapefile_path = path_vectorised_binary_filtered, report_band=5)

    # 6 minutes
    rb7_zstats_df = zonal_statistics(raster_path=change_report_path, shapefile_path = path_vectorised_binary_filtered, report_band=7)

    # table joins, area, lat lon, county
    geopandas_function(rb2_zstats_df = rb2_zstats_df, rb5_zstats_df = rb5_zstats_df, rb7_zstats_df = rb7_zstats_df,
